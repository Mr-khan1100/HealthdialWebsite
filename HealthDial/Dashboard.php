...
if($logRes) while($log = $logRes->fetch_assoc()) $activityLogs[] = $log;
...
if($ruRes) while($ru = $ruRes->fetch_assoc()) $recentUsers[] = $ru;
...
if($cityRes) while($c = $cityRes->fetch_assoc()) $topCities[] = $c;
...
for($i = 29; $i >= 0; $i--) {
    ...
    $dayCount = $conn->query("SELECT COUNT(*) as cnt FROM listings WHERE DATE(created_at) = '$date'")->fetch_assoc()['cnt'];
...
if($trRes) while($tr = $trRes->fetch_assoc()) $topRated[] = $tr;
        ...
        <div class="admin-main">
            ...
            <div class="admin-content">
                ...
                <div class="page-header">
                    ...
                    <p class="page-subtitle">Welcome back, <?php echo htmlspecialchars($_SESSION['admin_name']); ?>! Here's your overview.</p>
                </div>
                ...
                <div class="stat-grid">
                    ...
                    <div class="stat-card amber fade-in fade-in-delay-2">
                        <div class="stat-info">
                            ...
                            <p class="stat-value" data-count="<?php echo $pendingListings + $pendingReviewsCount; ?>">0</p>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap: 14px; margin-bottom: 28px;">
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px;">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(59,130,246,0.1);display:flex;align-items:center;justify-content:center;color:#3b82f6;font-size:14px;">
                            <i class="fas fa-newspaper"></i>
                        </div>
                        <div>
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">News Articles</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo $totalNews; ?></div>
                        </div>
                    </div>
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px;">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(16,185,129,0.1);display:flex;align-items:center;justify-content:center;color:#10b981;font-size:14px;">
                            <i class="fas fa-pills"></i>
                        </div>
                        <div>
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">Medications</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo $totalMedications; ?></div>
                        </div>
                    </div>
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px;">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(245,158,11,0.1);display:flex;align-items:center;justify-content:center;color:#f59e0b;font-size:14px;">
                            <i class="fas fa-file-alt"></i>
                        </div>
                        <div>
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">Documents</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo $totalDocuments; ?></div>
                        </div>
                    </div>
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px;">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(99,102,241,0.1);display:flex;align-items:center;justify-content:center;color:#6366f1;font-size:14px;">
                            <i class="fas fa-layer-group"></i>
                        </div>
                        <div>
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">Categories</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo count($catLabels); ?></div>
                        </div>
                    </div>
                    <a href="Notification.php" style="text-decoration:none;color:inherit;">
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px; cursor:pointer; transition:transform 0.2s;" onmouseover="this.style.transform='scale(1.03)'" onmouseout="this.style.transform='scale(1)'">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(239,68,68,0.1);display:flex;align-items:center;justify-content:center;color:#ef4444;font-size:14px;">
                            <i class="fas fa-bell"></i>
                        </div>
                        <div style="flex:1;">
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">Push Notifications</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo $totalPushNotifications; ?></div>
                        </div>
                    ...
                    <a href="Notification.php" style="text-decoration:none;color:inherit;">
                    <div class="card" style="padding: 16px 20px; display: flex; align-items: center; gap: 12px; cursor:pointer; transition:transform 0.2s;" onmouseover="this.style.transform='scale(1.03)'" onmouseout="this.style.transform='scale(1)'">
                        <div style="width:36px;height:36px;border-radius:8px;background:rgba(168,85,247,0.1);display:flex;align-items:center;justify-content:center;color:#a855f7;font-size:14px;">
                            <i class="fas fa-alarm-clock"></i>
                        </div>
                        <div style="flex:1;">
                            <div style="font-size:11px;color:var(--text-muted);font-weight:600;">Med Reminders</div>
                            <div style="font-size:20px;font-weight:800;color:var(--text-primary);"><?php echo $totalMedReminders; ?></div>
                        </div>
                        <div style="font-size:11px;color:#0782ca;font-weight:600;">View All →</div>
                    </div>
                    </a>
                ...
                <div style="display: grid; grid-template-columns: 1.2fr 0.8fr; gap: 20px; margin-bottom: 28px;">
                    <div class="card fade-in">
                        <div class="card-header">
                            ...
                            <span class="badge badge-success"><i class="fas fa-chart-line" style="margin-right:4px;"></i> Live</span>
                        </div>
                        <div class="card-body">
                                    ...
                                    <tr>
                                        <td>
                                            <div style="display:flex;align-items:center;gap:10px;">
                                                <div style="width:32px;height:32px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--accent));display:flex;align-items:center;justify-content:center;color:#fff;font-size:12px;font-weight:700;">
                                                    <?php echo strtoupper(substr($user['name'],0,1)); ?>
                                                </div>
                                                <div>
                                                    <div style="font-weight:600;color:var(--text-primary);font-size:13px;"><?php echo htmlspecialchars($user['name']); ?></div>
                                                    <div style="font-size:11px;color:var(--text-muted);"><?php echo htmlspecialchars($user['mobile'] ?? $user['email'] ?? ''); ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        <td><span class="badge badge-info"><?php echo htmlspecialchars($user['state'] ?? 'N/A'); ?></span></td>
                            ...
                            <div class="timeline">
                                ...
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        ...
                                        <?php if($log['details']): ?>
                                        <br><span style="color:var(--text-muted);font-size:12px;"><?php echo htmlspecialchars($log['details']); ?></span>
                                        <?php endif; ?>
                                    </div>
                                    <div class="timeline-time"><?php echo date('d M, h:i A', strtotime($log['created_at'])); ?></div>
                                </div>
                                <?php endforeach; ?>
                            </div>
                            <?php else: ?>
                ...
                <div style="margin-top:28px;">
                    <div class="page-header" style="margin-bottom:16px;">
                        <h2 class="page-title" style="font-size:18px;"><i class="fas fa-chart-area" style="color:#3b82f6;margin-right:8px;"></i>Website Analytics</h2>
                    </div>

                    <!-- Today's Quick Stats -->
                    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:20px;">
                        <div class="card" style="padding:20px;text-align:center;background:linear-gradient(135deg,rgba(16,185,129,0.05),rgba(16,185,129,0.02));">
                            <div style="font-size:28px;font-weight:800;color:#10b981;"><?= $todayListings ?></div>
                            <div style="font-size:12px;color:var(--text-muted);font-weight:600;margin-top:4px;">Listings Today</div>
                        </div>
                        <div class="card" style="padding:20px;text-align:center;background:linear-gradient(135deg,rgba(59,130,246,0.05),rgba(59,130,246,0.02));">
                            <div style="font-size:28px;font-weight:800;color:#3b82f6;"><?= $todayUsers ?></div>
                            <div style="font-size:12px;color:var(--text-muted);font-weight:600;margin-top:4px;">Users Today</div>
                        </div>
                        <div class="card" style="padding:20px;text-align:center;background:linear-gradient(135deg,rgba(245,158,11,0.05),rgba(245,158,11,0.02));">
                            <div style="font-size:28px;font-weight:800;color:#f59e0b;"><?= $todayReviews ?></div>
                            <div style="font-size:12px;color:var(--text-muted);font-weight:600;margin-top:4px;">Reviews Today</div>
                        </div>
                    </div>
                    ...
                    <div style="display:grid;grid-template-columns:1.5fr 1fr;gap:20px;">
                        ...
                        <div class="card fade-in">
                            <div class="card-header">
                                ...
                                <span class="badge badge-info"><i class="fas fa-chart-bar" style="margin-right:4px;"></i>30 Days</span>
                            </div>
                            <div class="card-body">
                        ...
                        <div class="card fade-in fade-in-delay-1">
                            ...
                            <div style="padding:0;">
                                ...
                                <div style="display:flex;align-items:center;gap:12px;padding:10px 20px;border-bottom:1px solid rgba(0,0,0,0.04);">
                                    <div style="width:24px;height:24px;border-radius:50%;background:<?= $idx < 3 ? '#3b82f6' : '#e2e8f0' ?>;color:<?= $idx < 3 ? '#fff' : '#64748b' ?>;font-size:11px;font-weight:700;display:flex;align-items:center;justify-content:center;"><?= $idx + 1 ?></div>
                                    <div style="flex:1;font-size:13px;font-weight:600;color:var(--text-primary);"><?= htmlspecialchars($city['city']) ?></div>
                                    <div style="font-size:12px;font-weight:700;color:#3b82f6;"><?= number_format($city['count']) ?></div>
                                </div>
                                <?php endforeach; ?>
                                <?php if(empty($topCities)): ?>
                                <div style="padding:30px;text-align:center;color:var(--text-muted);font-size:13px;">No city data available</div>
                                <?php endif; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card fade-in" style="margin-top:20px;">
                        ...
                        <div style="padding:0;">
                            <table class="data-table">
                                <thead><tr><th>Listing</th><th>City</th><th>Rating</th><th>Reviews</th></tr></thead>
                                <tbody>
                                    <?php foreach($topRated as $tr): ?>
                                    <tr>
                                        <td style="font-weight:600;"><?= htmlspecialchars($tr['name']) ?></td>
                                        <td><span class="badge badge-info"><?= htmlspecialchars($tr['city'] ?? 'N/A') ?></span></td>
                                        <td><span style="color:#f59e0b;font-weight:700;">⭐ <?= $tr['avg_rating'] ?></span></td>
