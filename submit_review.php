    ...
    try {
        $stmt = $conn->prepare(
            "INSERT INTO reviews (user_id, listing_id, rating, review, guest_name, status, created_at, updated_at)
             VALUES (NULL, ?, ?, ?, ?, 'approved', NOW(), NOW())"
        );
        if (!$stmt) {
            // Fallback: try without guest_name column
            $stmt = $conn->prepare(
...
// Handle concatenated JSON from backend
...
echo json_encode($result ?: ['success' => false, 'message' => 'API error (HTTP ' . $httpCode . ').']);
?>
